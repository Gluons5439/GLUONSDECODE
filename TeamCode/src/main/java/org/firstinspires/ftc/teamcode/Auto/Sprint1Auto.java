...
public class Sprint1Auto extends OpMode {
    ...
    private final Pose startPose = new Pose(66, 8, Math.toRadians(90));
    private final Pose shootPose = new Pose(66, 14, Math.toRadians(115));
    private final Pose frontCollect1 = new Pose(42,35.5, Math.toRadians(180));
    private final Pose collect1 = new Pose(20,35.5, Math.toRadians(180));
    ...
    public void statePathUpdate() {
        switch (pathState) {
            ...
            case DRIVE_FRONT_COLLECT:
                if (!pathStarted) {
                    // Do NOT reset pose here
                    follower.followPath(driveToFrontCollect, false);
                    pathStarted = true;
                }
                if (!follower.isBusy()) {
                    setPathState(PathState.COLLECT);
            ...
            case COLLECT:
                if (!pathStarted) {
                    follower.followPath(collectBallSet1, 0.5, false); // slow drive, no reset
                    pathTimer.resetTimer();
                    pathStarted = true;
                }
                ...
                // Stop intake and move to next path after 1.5s
                if (pathTimer.getElapsedTimeSeconds() >= 1.5) {
                    intake.setZaPower(0);
                    setPathState(PathState.SHOOT_SET_1);
                }
            ...
            case SHOOT_SET_1:
                if (!pathStarted) {
                    // Do NOT reset pose here
                    follower.followPath(driveToCollect1Shoot, false);
                    StopperServo.setPosition(STOPPER_UP_POS);
                    shooter.fireShots(3);
                    shotsTriggered = true;
                    pathStarted = true;
    ...
    public void init()
    {
        ...
        turretMoveMotor = hardwareMap.get(DcMotorEx.class, "turretMotor");
        turretMoveMotor.setDirection(DcMotorSimple.Direction.FORWARD);
        turretMoveMotor.setMode(DcMotorEx.RunMode.STOP_AND_RESET_ENCODER);
        turretMoveMotor.setZeroPowerBehavior(DcMotorEx.ZeroPowerBehavior.BRAKE);
