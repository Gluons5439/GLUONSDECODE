...
import com.qualcomm.robotcore.eventloop.opmode.OpMode;
import com.qualcomm.robotcore.eventloop.opmode.TeleOp;
import com.qualcomm.robotcore.hardware.CRServo; // Added
import com.qualcomm.robotcore.hardware.DcMotorSimple; // Added
import com.qualcomm.robotcore.hardware.Servo;
import com.qualcomm.robotcore.hardware.DcMotor;
import com.qualcomm.robotcore.util.Range;
...
import org.firstinspires.ftc.teamcode.pedroPathing.Constants;
// NOTE: Depending on your specific Pedro Pathing V2 setup, you may need to adjust the Follower
// initialization in init() to use FConstants/LConstants if that's how your system is configured.

@Configurable
@TeleOp(name = "Final Field-Centric TeleOp")
public class FinalFieldCentricDrive extends OpMode {
    ...
    // --- Shooter Variables ---
    private DcMotor shooterMotor;
    private static final double SHOOTER_MAX_POWER = -1.0; // Updated to -1.0 (from new code)
    private boolean shooterActive = false; // NEW toggle state variable

    // --- Intake Variables ---
    private DcMotor intakeMotor; // NEW motor
    private static final double INTAKE_POWER = -1.0; // NEW constant

    // --- Transfer Servos Variables ---
    private CRServo transferServo1; // NEW CR Servo
    private CRServo transferServo2; // NEW CR Servo
    private boolean transferActive = false; // NEW toggle state variable
    private static final double TRANSFER_STOP_POS = 0.0;
    private static final double TRANSFER_MOVE_POS = 1.0;

    // --- Vision/Limelight Variables ---
    private Limelight3A limelight;

    // --- Auto-Aim PID Constants (Retained from original code) ---
    private static final double AIM_KP = 0.0008;
    private static final double AIM_DEADZONE = 1.5;
    private static final double MAX_CORRECTION = 0.001;

    // --- Toggle variables ---
    // Turret master toggle removed: Button B is now the Shooter Toggle.
    private boolean isAutoAimActive = false;
    private boolean a_was_pressed = false;

    private boolean b_was_pressed = false; // For shooter toggle
    private boolean left_bumper2_was_pressed = false; // For transfer toggle
    ...
    public void init() {
        ...
        // Initialize Transfer CR Servos (NEW)
        try {
    ...
    public void loop() {
        ...
        // Retained from original code (using V2 setTeleOpDrive and 'false' for field centric)
        if (follower != null) {
            follower.setTeleOpDrive(
                    -gamepad1.left_stick_y,   // Translational power (Forward/Backward)
                    -gamepad1.left_stick_x,   // Strafe power (Left/Right)
        ...
        // --- AUTO-AIM TOGGLE (GamePad2 A) ---
        // Turret is now always active; A toggles aiming mode.
        boolean a_is_pressed = gamepad2.a;
        ...
        // --- MECHANISM CONTROL (NEW FUNCTIONS) ---
        handleShooterControl();   // Uses Gamepad 2 B (Shooter Toggle)
        handleIntakeControl();    // Uses Gamepad 1 Bumpers
        handleTransferControl();  // Uses Gamepad 2 Left Bumper
        ...
        telemetry.addData("Transfer Active (L Bumper 2)", transferActive);
    ...
    //
    // ---------- TRANSFER CONTROL (GamePad2 Left Bumper - TOGGLE) ----------
    //
    private void handleTransferControl() {
        ...
        left_bumper2_was_pressed = left_bumper2_is_pressed;

        double power = transferActive ? TRANSFER_MOVE_POS : TRANSFER_STOP_POS;

        // Set power for Continuous Rotation Servos
        transferServo1.setPower(power);
        transferServo2.setPower(-power); // Assuming opposite direction for second servo
    }
    ...
    //
    private void handleTurretControl() {
        // Direct position map: -1.0 -> 0.0, 0 -> 0.5, 1.0 -> 1.0
        double turretMoveInput = gamepad2.left_stick_x;
        turretTargetPosition = (turretMoveInput + 1.0) / 2.0;
    }
    ...
    public void stop() {
        ...
        if (transferServo1 != null) transferServo1.setPower(TRANSFER_STOP_POS);
        if (transferServo2 != null) transferServo2.setPower(TRANSFER_STOP_POS);
    }
}