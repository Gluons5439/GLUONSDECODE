...
public class TestFieldDrive3 extends OpMode {
    ...
    public void loop() {
        ...
        // --- MECHANISM CONTROL ---
        handleShooterControl();   // Gamepad 2 B
        handleIntakeControl();    // Gamepad 1 & 2 Bumpers
        handleTransferControl();  // Gamepad 2 Left Bumper
    ...
    private void handleIntakeControl() {
        ...
        // Check for Intake Forward (Suck In)
        boolean intakeForward = gamepad1.right_bumper || gamepad2.right_bumper;

        // Check for Intake Reverse (Spit Out)
...
public class TestFieldDrive3 extends OpMode {
    ...
    // --- Transfer Servo 2 Variables (One-Shot) ---
    private Servo transferServo2;
    private int transferState = 0; // 0=Down, 1=Up/Delaying
    private long sequenceStartTime = 0;
    private static final long UP_DURATION_MS = 300; // Time (in milliseconds) the servo stays up
    private static final double TRANSFER_DOWN_POS =0.5; // Resting/Down Position
    private static final double TRANSFER_UP_POS = 1; // Flipped/Up Position

    // --- Transfer Servo 1 Variables (NEW - D-Pad Control) ---
    private Servo transferServo1; // NEW SERVO
    private double transfer1TargetPosition = 0.0; // Starts at 0.0 (Straight)
    private static final double TRANSFER1_STRAIGHT_POS = 0.0;
    private static final double TRANSFER1_UP_POS = 1.0;
    private boolean dpad_up2_was_pressed = false; // For transferServo1 control
    private boolean dpad_down2_was_pressed = false; // For transferServo1 control
...
// Initialize Transfer Servo 2 (Existing One-Shot)
        try {
            transferServo2 = hardwareMap.get(Servo.class, "Transfer2");
            transferServo2.setPosition(TRANSFER_DOWN_POS); // Set to resting position
        } catch (Exception e) {
...
// Initialize Transfer Servo 1 (NEW D-Pad Controlled)
        try {
            transferServo1 = hardwareMap.get(Servo.class, "transferServo1"); // Use the requested name
            transferServo1.setPosition(TRANSFER1_STRAIGHT_POS); // Set to starting position (0.0)
        } catch (Exception e) {
...
// --- MECHANISM CONTROL ---
        handleShooterControl(); // Gamepad 2 B
        handleIntakeControl(); // Gamepad 1 & 2 Bumpers
        handleTransferControl2(); // Gamepad 2 Left Bumper (Existing one-shot)
        //handleTransferControl1(); // Gamepad 2 D-Pad Up/Down (NEW)
...
// --- TELEMETRY ---
        ...
        telemetry.addData("Shooter Active (B)", shooterActive);
        telemetry.addData("Intake Control", "R/L Bumpers 1 & 2");
        telemetry.addData("Transfer 2 State", transferState == 0 ? "DOWN (Ready)" : "UP (Sequencing)");
        telemetry.addData("Transfer 1 Pos (D-Pad)", transfer1TargetPosition);
        telemetry.addData("Robot Pose", follower != null ? follower.getPose() : "N/A");
    ...
    //
// ---------- TRANSFER CONTROL 1 (GamePad2 D-Pad Up/Down - NEW TOGGLE) ----------
//
     /*
    private void handleTransferControl1() {
        if (transferServo1 == null) return;

        boolean dpad_up2_is_pressed = gamepad2.dpad_up;
        boolean dpad_down2_is_pressed = gamepad2.dpad_down;

// D-Pad Up: Go to position 1.0 (Straight Up)
        if (dpad_up2_is_pressed && !dpad_up2_was_pressed) {
            transfer1TargetPosition = TRANSFER1_UP_POS; // 1.0
        }
        dpad_up2_was_pressed = dpad_up2_is_pressed;

// D-Pad Down: Go to position 0.0 (Straight)
        if (dpad_down2_is_pressed && !dpad_down2_was_pressed) {
            transfer1TargetPosition = TRANSFER1_STRAIGHT_POS; // 0.0
        }
        dpad_down2_was_pressed = dpad_down2_is_pressed;

// Apply the position
        transferServo1.setPosition(transfer1TargetPosition);
    }
*/
    //
// ---------- TRANSFER CONTROL 2 (GamePad2 Left Bumper - EXISTING ONE-SHOT) ----------
//
    private void handleTransferControl2() { // Renamed from handleTransferControl for clarity
        if (transferServo2 == null) return;

        boolean left_bumper2_is_pressed = gamepad2.dpad_left;
        boolean right_d_pad_is_pressed = gamepad2.dpad_right;
...
// State 1: Up and Waiting for Timer
            ...
            if (right_d_pad_is_pressed) {
// 2. Timer elapsed, go back Down
                transferServo2.setPosition(1);
            }
        }
...
//
    ...
    private void handleAutoAim() {
        ...
        if (result != null && result.isValid()) {
            ...
            if (Math.abs(tx) > AIM_DEADZONE) {
// Proportional correction calculation and clipping
                double correction = Range.clip(tx * AIM_KP, -MAX_CORRECTION, MAX_CORRECTION);
                turretTargetPosition -= correction;
                telemetry.addData("Target X (°)", "%.2f", tx);
                telemetry.addLine("Auto-Aim: Tracking...");
...
// Stop all active mechanisms
        ...
        if (intakeMotor != null) intakeMotor.setPower(0);
        if (transferServo1 != null) transferServo1.setPosition(TRANSFER1_STRAIGHT_POS);
        if (transferServo2 != null) transferServo2.setPosition(TRANSFER_DOWN_POS);
    }
}